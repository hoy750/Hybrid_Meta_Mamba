...
def run(config: dict) -> None:
    rows = []
    comparisons = {
        "without_mamba": [name for name in config["base_learners"] if name != "mamba"],
        "with_mamba": config["base_learners"],
    }
    for stage in range(1, 5):
        ...
        for label, learners in comparisons.items():
            ...
            output_dir = f"{config['outputs']['results_dir']}/remove_mamba/{label}/{stage_name}/meta_mamba"
            ...
            rows.append(metrics)
        save_metrics_table(rows, f"{config['outputs']['results_dir']}/remove_mamba/metrics.csv")


def main() -> None:
    parser = argparse.ArgumentParser(description="Experiment 6: remove Base Mamba ablation.")
