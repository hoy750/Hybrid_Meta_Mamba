...
def run(config: dict) -> None:
    ...
    for stage in range(1, 5):
        ...
        for k in config["topk_values"]:
            ...
            output_dir = f"{config['outputs']['results_dir']}/topk/top{k}/{stage_name}/meta_mamba"
            ...
            rows.append(metrics)
        save_metrics_table(rows, f"{config['outputs']['results_dir']}/topk/metrics.csv")


def main() -> None:
    parser = argparse.ArgumentParser(description="Experiment 3: top-k base learner ablation.")
